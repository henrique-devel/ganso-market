...
// RFC-010 stop condition and mandatory test: this module estimates and nothing
// else. A code search over every file it owns must find no trading auth, no
// wallet or signer, no private key, and no order path — not even a disarmed or
// feature-flagged one. If a future change adds any of them, this test fails
// before the change can ship.

const MODULE_ROOT = new URL(
  "../../../src/polymarket/fundamental/",
  import.meta.url,
).pathname;

const ENTRYPOINT = new URL(
  "../../../src/polymarket-estimator.ts",
  import.meta.url,
).pathname;
...
/** Tables this module is allowed to write to. */
...
describe("RFC-010 scope", () => {
  it("contains no trading auth, wallet, signer or order path", async () => {
  ...
  it("never imports the auth service or any network client", async () => {
    ...
    for (const file of files) {
      const source = stripComments(await readFile(file, "utf8"));
      // The API route file legitimately depends on the RFC-002 auth service to
      // GUARD its endpoints; nothing else in the module may touch auth, and no
      // file may open a socket or make an outbound request.
