...
export async function applyIoc(
  ...
  const request: IocCommand = JSON.parse(JSON.stringify(input)),
    scope = { ...scopeInput };
  return withRisk(pool, scope, true)((tx) => applyIocTx(tx, scope, request));
}
/** Same transaction/owner lock as the desk command receipt; never a new pool. */
export async function applyIocTx(
  tx: SqlExecutor,
  scopeInput: TradingScope,
  input: IocCommand,
): Promise<IocResult> {
  ...
  if (request.action === "submit") {
    ...
    requireIoc(
      !!meta &&
        ...
        BigInt(meta.fees.taker.raw) <= BigInt(request.order.fee_bps) * 100_000n,
  ...
  if (request.action !== "submit") {
    if (reservation.status !== "active") {
      ...
      let reason = request.action === "cancel" ? "requested" : "ioc_remainder";
      let fills: IocFill[] = [];
      if (request.action === "execute" && reservation.order.valid_until > now) {
