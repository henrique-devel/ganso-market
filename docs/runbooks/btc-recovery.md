...
| Release default, orçamento 45 min | Canal existente `.github/workflows/ci-cd.yml`, arquivo por `git archive --format=tar.gz --output="$OPS_ARCHIVE" "$OPS_RELEASE"`, envio SSH `"deploy $OPS_RELEASE" < "$OPS_ARCHIVE"` pela chave restrita já instalada | `remote-deploy.sh` valida arquivo ≤50 MiB, guarda código e chama `make server-update`: build → postgres sem force → migrate → consumidores ativos afetados com `--no-deps`, aposentadoria seletiva dos stubs G2-03.4 → probes. Não rodar manualmente o script sem o contrato de SSH. Registrar hash/saída do job. Rollback automático só de código/default, migrations permanecem. |
