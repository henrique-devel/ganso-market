Bloco: EXEC-02 | RFC: RFC-034 | Data UTC: 2026-09-16
Estado: code-verified; validação local, publicação/produção ainda não verificadas.
Código: base e400a70 (EXEC-01/PR189); branch codex/exec-02-final-order, worktree /private/tmp/ganso-exec02.
...
Recotação: snapshot e parâmetros relidos na transação; parâmetros/mapeamento bloqueados até reserva; nenhuma mutação da ordem após avaliação.
...
Checks: make verify passou (2456 JS, 262 PG skips separados; 16 Rust, 220 Python, build/lint/scan/Compose); três testes adicionais de fonte passaram.
SQL: make test-postgres passou, PostgreSQL18.4 descartável/25 migrations, 23 arquivos/293 testes/zero falhas/zero skips.
