...
| --- | --- | --- | --- | --- | --- |
| 1 | [#133](https://github.com/henrique-devel/ganso-market/pull/133), squash em `main` 2026-09-08 18:30Z, `061f1ba16697dcfa65853f5dad1fca6f9c6ec23a` | `/opportunities` **281,1 ms** (quente 8,6 ms); `/decisions` JOIN **5,7 ms** com planejamento de 92,4 ms (quente 0,68 ms, Memoize: 54 buscas de índice e 446 acertos em 500 linhas); bloco `config` **0,29 ms**. Nada perto dos 500 ms, então o JOIN fica na API | **A1** n/a (nenhum location novo). **A2 ✔** `release-sha` no container da `api` **e** no bundle do `web`. **A3 ✔** 200/200 do painel e 500/500 do log com nome; 0 `question` nula com registro, 0 fora do registro, 0 sem categoria. **A4 ✔** por teste de texto renderizado (2 casos, um deles visto falhando com `' Normal NORMAL '`). **A5 PENDENTE, depende do proprietário** — ver premissa caída. **A6 ✔** (coluna à esquerda). **A7 0** em 24 h agora; refazer em 09/09 ~18:35Z | nenhum (inalterado e verde: 61 testes) | **(1) O orçamento da D5 estava errado por 4:** `fetchDashboardStatus` faz **duas** requisições por tique (`health.ts:35-36`), então "status do App 4" são 8 e a soma dada como 18 era **22** — acima do teto do A5. Corrigido movendo status e feed de eventos para rodarem só no Sistema; a Mesa fecha em **14 req/min**. **(2) O A5 não é executável como está escrito:** o `log_format ganso_json` (`infra/nginx/nginx.conf:11-15`) não registra URI nem IP — só instante, método, status e tempo. Substituto: contar todas as requisições do edge em 60 s com a Mesa aberta (`grep -c http_request_completed`), o que exige uma sessão do painel e portanto o proprietário. **(3) Desvio da D6, deliberado:** o bloco `config` sai de `portfolio_config_versions` e não da config carregada pela API, porque o serviço `api` não monta `config/portfolio.json` nem define `GANSO_PORTFOLIO_CONFIG_FILE` — ali `loadPortfolioConfig()` devolveria os defaults compilados, que é o hardcode que a D6 remove, uma camada atrás. **(4) Medições novas:** o livro do `panel_json` tem no máximo **10 níveis por lado** (200 snapshots) e a amostra de 500 decisões cobre **26 min** no ritmo atual, o que limita o alcance da frase de aceites sem ordem da D7 |
| 2 | [#135](https://github.com/henrique-devel/ganso-market/pull/135), squash em `main` 2026-09-08 19:40Z, `12d4edcd0d9ea4a20f10addcf365575193d28820` | **Os dois filtros estouraram o teto e foram para o fallback da própria D9.** `?outcome='ACCEPTED'` **2 595 ms a frio** e **1 992 ms a quente**; `'REJECTED'` 2,3 ms; `?condition_id=` **p95 a frio 1 104,7 ms** em 9 mercados (1104,7 / 498,5 / 390,4 / 99,2 / 97,6 / 89,3 / 57,9 / 42,0 / 26,2), 8,9 ms a quente. Sem índice em `outcome` o planejador faz *parallel seq scan* e lê **43 248 blocos do disco nas DUAS execuções** — a tabela não cabe no `shared_buffers`, e é por isso que "a quente" quase não ajuda. Rotas novas medidas: `/paper/positions` **1,5 ms**, `/paper/orders` **1,5 ms** (orçamento declarado: 500 ms cada) | **A1 ✔** de dentro do servidor: GET `positions` e `orders` **401**, POST e DELETE nos dois **404**, `/paper/intents` **404** nos três métodos. **A2 ✔** `release-sha` no container da `api` = `12d4edc…`, igual ao SHA do merge. **A6 ✔** com fallback registrado (coluna à esquerda): nada de índice, nada de orçamento maior. **A7 0** pelo método A3 da RFC-023 (`docker logs ganso-market-api-1 --since 24h | grep -c '"pg_code":"57014"'`); refazer em 09/09 ~19:45Z. O log do postgres mostra 7 cancelamentos em 24 h, **todos anteriores a este deploy e de outro serviço**: 1 `SELECT pg_sleep(2)` e 6 do `SELECT DISTINCT ON (token_id) … bids_json` de `portfolio/exitstore.ts:673`, que roda no worker `polymarket-portfolio` — é exatamente a mistura que o A3 da RFC-023 existe para desfazer. **A3/A4/A5** não se aplicam a este PR | `:36` PAPER_ALLOWLIST de 2 → 4 entradas; `:96-111` cobre os dois novos; `:113-123` perde `orders` e `positions` e **mantém** `intents`, `kill-switch`, `portfolio/halt` e `portfolio/resume`; caso "prefixo sob `/paper`" continua reprovando. Teste novo: o bloco declara **um** guard e ele é `GET`. 9 testes OK | **(1) A D9 caiu:** os dois filtros passam do teto de 500 ms **e** do `statement_timeout` de 1 s da API, então publicá-los daria 57014 em toda carga da tela padrão (Aceitas, P5) em vez de uma tela lenta — contra o A7. `/decisions` **não ganha parâmetro nenhum** e o filtro fica no cliente sobre as últimas 500. Consequência que a tela precisa dizer: no ritmo de 08/09 as últimas 500 decisões contêm **~4 aceites** e cobrem ~26 min. O teste obrigatório "`?outcome=X` → 400" fica **sem objeto**: o parâmetro não existe. **(2) Contagens remedidas:** 143 `ACCEPTED` × 18 931 `REJECTED` em 24 h (eram 94 × 52 868 em 02/09); `queue_ahead` em **63 de 63** ordens (eram 18 de 18); 184 370 decisões no total. **(3) `is_final` não discrimina:** as 2 942 linhas de `fundamental_labels` têm **todas** `is_final = true`, então o selo vale "tem rótulo E cotas > 0". Em produção hoje ele **não acende em nenhuma** das 6 posições — as 3 rotuladas estão com `shares = 0` e as 3 com saldo não têm rótulo. O selo está armado e silencioso; quem prova que ele dispara é o teste pg. **(4) Desvio deliberado da D8:** `end_ts` das posições sai de `COALESCE(f.end_date, m.end_ts)` e não de `m.end_date_iso`, seguindo a precedência da RFC-016 que `/opportunities` já usa — `end_date_iso` é a cópia só-data que a migration 0017 existe para desaconselhar, e `m.end_ts` sozinho tinha instante para 219 de 372 tokens. **(5) Guarda que faltava:** `route-budgets.test.ts` (RFC-023 D1) reprova toda rota publicada sem orçamento declarado, então `config/runtime.json` ganhou as duas entradas. Não estava no escopo escrito do PR e é pré-requisito de merge |
| 3 | [#137](https://github.com/henrique-devel/ganso-market/pull/137), squash em `main` 2026-09-08 22:30Z, `93679126a9eadda80964c4fe27468b9a619eac30` | **Com os tetos em pé:** lote `?tokens=` **25×60 = 108,9 ms** a frio (pior de 12 execuções; 0,6 ms a quente; p95 305,1 ms em 9 janelas frias distintas) e **1 token × 720 buckets (12 h) = p95 77,5 ms**. O 1×1 440 que a D10 pedia mediu 125,9 ms na primeira leitura mas **p95 700,1 ms em 24 amostras nos tokens mais densos, 4 delas acima de 500 ms** — por isso o gráfico é de 12 h e não de 24 h. **Sem teto de janela, que é o que a D10 escrevia:** 1 token × 7 dias **3 811,6 ms**, × 20 dias **14 699,1 ms**, lote 25 × 24 h **612,4 ms**. Causa: `shared_buffers` 128 MB contra 1 735 MB de `polymarket_series_1m`, com ~200 tokens gravados intercalados — 24 h de um token são ~1 400 páginas espalhadas | **A1 ✔** de dentro do servidor: GET **401** nas duas rotas novas, **404** em POST/PUT/DELETE nas duas, `/paper/intents` **404** nos três métodos. **A2 ✔** `release-sha` da `api` = `9367912…`. **A6 ✔** (coluna à esquerda). **A7 0** pelo método A3 da RFC-023, e 0 `read_api_failed` em 24 h; os 7 `canceling statement` do postgres são os mesmos do PR 2, todos anteriores a este deploy e do worker `polymarket-portfolio`. Refazer em 09/09 ~22:35Z. **A5 PENDENTE DO PROPRIETÁRIO** pelo mesmo motivo do PR 1, re-conferido: o `log_format` não registra URI nem IP. Por construção a Mesa vai de 16 a **18 req/min**, e o lote é UMA requisição por tique qualquer que seja o tamanho da página | `:96-111` inalterado; **dois testes novos**: o prefixo `^~ /api/polymarket/series` existe uma única vez, declara **um** guard e ele é `GET`; e ele não está sob `/paper`. 9 → **11 testes OK**, com o primeiro visto falhando no `nginx.conf` anterior | **(1) A premissa central da D10 caiu:** `from` obrigatório **não limita nada** — o `from` é do cliente, e um `from` distante devolve a varredura inteira (números à esquerda). O teto teve de ser a **janela**: 60 min no lote e 12 h no `ohlc`, cada uma a que a própria tela pede; janela maior é **400** e não resposta lenta, porque resposta lenta vira 57014 contra o A7. **(2) O gráfico de 24 h da D10/D5 não cabe no orçamento** e ficou em **12 h** (p95 700 ms contra 77,5 ms). **(3) Metodologia:** medir aquece o cache e não se larga o cache de um Postgres em produção — as leituras confiáveis são as PRIMEIRAS de cada forma de consulta, e onde as leituras discordaram o teto foi escolhido pela pessimista. **(4) Remedições:** `polymarket_series_1m` tem 4 045 353 linhas e **1 735 MB**; **164** tokens com bucket na última hora (eram 210 em 02/09); ~6 % dos buckets da última hora têm mid **nulo**, então a tela trata nulo como lacuna e não como zero. **(5) Guarda que faltava, de novo:** `route-budgets.runtime.test.ts` não conseguia verificar rota que recusa com 400 antes de consultar — ganhou query string por rota. E `config/runtime.json` ganhou os dois orçamentos no mesmo commit (RFC-023 D1). **(6) Fora do escopo, não corrigido:** `fundamental/config.test.ts` falha com `--no-file-parallelism` e passa no `make verify`; confirmado pré-existente no HEAD, é vazamento de env entre arquivos de teste |
