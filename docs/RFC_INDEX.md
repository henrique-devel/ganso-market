...
| [RFC-021](rfcs/RFC-021-silencio-do-feed-e-kill-switch.md)           | Silêncio do feed com conexões vivas e kill switch honesto | RFC-007, RFC-011, RFC-020        | parada silenciosa do WS vira lacuna `stream_silent`; gatilho lê deltas e snapshots e diz qual série calou; `closed` reflete a venue — **accepted (2026-09-04)**; decisões aprovadas em 2026-09-05 **menos a D3**: o rearme automático condicionado (P1) **não** foi aprovado e o rearme segue manual |
...
| [RFC-029](rfcs/RFC-029-tela-sombra-shadow-replay.md)                | Tela Sombra: shadow replay por job diário               | RFC-017 (emenda leve), RFC-015, RFC-026 | job no host grava JSON em volume só-leitura; endpoints GET que só leem disco; tela com ressalvas fixas e sem promoção — **accepted (2026-09-04)**; P1–P3 concedidas em 2026-09-05 |

> **2026-09-05 — RFC-020…029: todas aprovadas**, cada decisão na recomendação/padrão da
> sua própria tabela. Registro item a item em `docs/HANDOFF.md`, seção "APROVAÇÃO DAS
> RFC-020…029". **Aprovada ≠ implementada:** nenhuma das dez está em produção. Única
> exceção à aprovação em bloco: a **D3 da RFC-021** (rearme automático condicionado)
> **não** foi aprovada — o rearme do kill switch segue manual e é ato do proprietário.
