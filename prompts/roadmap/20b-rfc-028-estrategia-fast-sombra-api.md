...
| PR-0, RFC-022, RFC-025 | **PR-0 implementado e verificado em produção** (PRs #93, #94, #96, 04/09); RFC-022 e RFC-025 em **`accepted`** e **ainda não implementadas** (`RFC-022…:3`, `RFC-025…:3`) | PR-0: `count(*) FROM paper_ledger_events WHERE event_type='resolution'` > 0; RFC-025: `count(*) FROM portfolio_circuit_breakers WHERE kind='PARAM_CHANGE' AND ended_at IS NULL` = 0 em mercados vivos; RFC-022: status no repo. **Ausente ⇒ PARE antes do merge do PR 3** |
...
- Dependência (PR-0, RFC-022, RFC-025) ausente em produção: PARE antes do merge do PR 3. Em 04/09 o **PR-0 já está de pé**; quem pode faltar é RFC-022 ou RFC-025 — re-meça as três mesmo assim.
